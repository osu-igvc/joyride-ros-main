...
class PointCloudPublisher(Node):
    def __init__(self):
        ...
        self.calibration_file = self.declare_parameter('calibration_file', 'calibration_center.csv').get_parameter_value().string_value
        self.image_sub_topic = self.declare_parameter("subscriber_topic", "/perception/lane/edge").get_parameter_value().string_value
    ...
    def calibrate_uv_xyz_transform(self,  calibration_type: str, calibration_file: str) -> np.ndarray:
        if calibration_type == "correlated":
            ...
            R = np.transpose([np.ones_like(U), U, U**2, U**3, V, V**2, V**3, U*V, U*V**2, U**2 * V])
    ...
    #     msg = PointCloud2()
    #     msg.header.stamp = self.get_clock().now().to_msg()
    #     msg.header.frame_id = f'{self.camera_frame}_objects'
    #     msg.height = 1
