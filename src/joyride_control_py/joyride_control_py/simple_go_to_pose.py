...
from geometry_msgs.msg import PoseStamped, Twist, Pose, Pose2D, Point
...
class SimpleGoToPose(Node):
    def __init__(self):
        ...
        self.proximity = self.declare_parameter("proximity", 0.5).get_parameter_value().double_value
        self.L = self.declare_parameter("wheelbase", 1.75).get_parameter_value().double_value
        self.v_max = self.declare_parameter("maximum_linear_speed", 2.0).get_parameter_value().double_value
        self.ω_max = self.declare_parameter("maximum_angular_speed", np.pi/2).get_parameter_value().double_value

        odom_topic = self.declare_parameter("odom_topic","odom").get_parameter_value().string_value
        plan_topic = self.declare_parameter("plan_topic","plan").get_parameter_value().string_value
        cmd_topic = self.declare_parameter("cmd_topic","cmd_vel_simple").get_parameter_value().string_value

        self.odomSub = self.create_subscription(Odometry, "/"+odom_topic, self.odometry_cb, 10)
        self.pathSub = self.create_subscription(Path, "/"+plan_topic, self.path_cb, 10)
        self.goalPub = self.create_publisher(Point,"/controller/simple/goal_pose",10)
        
        self.goalPubMsg = Point()
        
        self.controlTimer = self.create_timer(0.01,self.controller_cb)
        
        self.cmd_vel_pub = self.create_publisher(Twist,"/"+cmd_topic, 1)
    ...
    def path_cb(self, msg: Path) -> None:
        ...
        goal_final = self.path_poses[-1].pose
        q = goal_final.orientation
        R = Rotation.from_quat([q.x, q.y, q.z, q.w])
        self.goal_final.x = goal_final.position.x
        self.goal.y = goal_final.position.y
    ...
    def increment_goal(self) -> None:
        ...
        self.goalPubMsg.x = self.goal.x
        self.goalPubMsg.y = self.goal.y
        self.goalPub.publish(self.goalPubMsg)
    ...
    def controller(self) -> None:
        ...
        ρ_f, α_f, β_f = self.tracking_errors(self.current_pose, self.goal_final)
 
        if ρ_f < self.proximity:
            self.cmd_vel_msg.linear.x = 0.0
            self.cmd_vel_msg.angular.z = 0.0

        elif ρ < self.proximity:
            self.increment_goal()
