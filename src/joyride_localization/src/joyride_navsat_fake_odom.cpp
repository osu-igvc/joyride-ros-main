...
// 3. Use only for fake odom to complete correct TF for indoor testing of Nav2 
...
{
NavSatFakeOdom::NavSatFakeOdom(const rclcpp::NodeOptions & options) : Node("navsat_fake_odom_node", options),
...
NavSatFakeOdom::~NavSatFakeOdom()
{
    //RCLCPP_INFO(this->get_logger(), "Shutting down NavSatFakeOdom node");
...
// --------------- Setup --------------------- //
void NavSatFakeOdom::initializeParameters()
{
    //RCLCPP_INFO(this->get_logger(), "Init params");
    this->declare_parameter("use_fake_odom", true);
...
void NavSatFakeOdom::getInitialLLCallback(const std::shared_ptr<joyride_interfaces::srv::GetOdomOriginLL::Request> request, std::shared_ptr<joyride_interfaces::srv::GetOdomOriginLL::Response> response)
{
...
void NavSatFakeOdom::initializeROS()
{
    ...
    this->publishOdometryTimer_ = this->create_wall_timer(std::chrono::milliseconds(int(1000 * tf_period_)), std::bind(&NavSatFakeOdom::publishOdometryCallback, this));

    this->getOdomOriginLLService_ = this->create_service<joyride_interfaces::srv::GetOdomOriginLL>("getInitialLL", std::bind(&NavSatfakeOdom::getInitialLLCallback, this, std::placeholders::_1, std::placeholders::_2));
    this->resetOdomService_ = this->create_service<std_srvs::srv::Trigger>("resetOdom", std::bind(&NavSatFakeOdom::resetOdomCallback, this, std::placeholders::_1, std::placeholders::_2));
    ...
    if(!this->use_fake_odom_) {
        ...
        this->gpsCommonSub_ = this->create_subscription<vectornav_msgs::msg::CommonGroup>(this->gps_common_topic_, 10,
            std::bind(&NavSatFakeOdom::gpsCommonCallback, this, std::placeholders::_1));

    }

}

void NavSatFakeOdom::resetOdomCallback(const std::shared_ptr<std_srvs::srv::Trigger::Request> request, std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
...
void NavSatFakeOdom::gpsCommonCallback(const vectornav_msgs::msg::CommonGroup::SharedPtr msg)
{
...
// Update measurements 
void NavSatFakeOdom::updateTransform(const vectornav_msgs::msg::CommonGroup::SharedPtr msg)
{
...
void NavSatFakeOdom::diagnosticCallback(diagnostic_updater::DiagnosticStatusWrapper &stat)
{
    if (this->valid_fix_obtained_)
    {
...
nav_msgs::msg::Odometry NavSatFakeOdom::buildOdometryMessage(const geometry_msgs::msg::Point::SharedPtr position, const geometry_msgs::msg::Quaternion::SharedPtr orientation, const geometry_msgs::msg::Twist::SharedPtr velocity, const std::string frame_id, const std::string child_frame_id)
{
...
geometry_msgs::msg::TransformStamped NavSatFakeOdom::buildOdometryTransform(const geometry_msgs::msg::Point::SharedPtr position, const geometry_msgs::msg::Quaternion::SharedPtr orientation, const std::string frame_id, const std::string child_frame_id)
{
...
void NavSatFakeOdom::publishOdometryCallback()
{
...
double NavSatFakeOdom::llDistance(double lat1, double lon1, double lat2, double lon2)
{
